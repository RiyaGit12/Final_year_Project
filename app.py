...
st.caption("Analyze health text & detect outbreaks")
...
# ---------------- TABS ----------------
tab1, tab2, tab3 = st.tabs(["📊 Dashboard", "🔍 Analyze", "🚨 Outbreak"])
...
with tab1:
    ...
    col2.metric("Negative", label_counts.get('Negative', 0))
    col3.metric("Neutral", label_counts.get('Neutral', 0))
    col4.metric("Positive", label_counts.get('Positive', 0))
...
with tab2:
    ...
    user_input = st.text_area("Enter health-related text")
...
with tab3:
    ...
    if st.button("Check"):
        # Dummy logic (simple for UI)
        st.info(f"No major outbreak detected in {country}")